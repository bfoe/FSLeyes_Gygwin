...
# download missing scripts from FSLeyes_Gygwin repository
if [ ! -f "Setup_FSLeyes.sh" ]; then
  wget -q https://raw.githubusercontent.com/bfoe/FSLeyes_Gygwin/master/Setup_FSLeyes.sh
...
if [ ! -f "/usr/local/lib/libspatialindex.dll.a" ]; then
  echo "Installing spatialindex library"
  ...
  cmake . 
  make install; cd ..
  rm spatialindex-src-1.9.3.tar.gz; tar -zcvf spatialindex-src-1.9.3.tar.gz spatialindex-src-1.9.3
  mkdir ~/backuped>/dev/null; cp spatialindex-src-1.9.3.tar.gz ~/backuped
...
echo "Installing fsleyes extras"
